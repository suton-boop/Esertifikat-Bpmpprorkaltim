...
<div class="card border-0 shadow-sm rounded-4 mb-4">
  <div class="card-body">
    <form method="GET" action="<?php echo e(route('admin.tte.signing.index')); ?>" class="row g-3 align-items-end">
      <div class="col-md-5">
        <label class="form-label small text-muted mb-1">Pencarian</label>
        <input type="text" class="form-control" name="q" value="<?php echo e($q ?? ''); ?>" placeholder="Cari No. sertifikat atau nama peserta...">
      </div>
      <div class="col-md-5">
      ...
      <div class="col-md-2 d-flex gap-2">
        ...
        <a href="<?php echo e(route('admin.tte.signing.index')); ?>" class="btn btn-outline-secondary" title="Reset filter"><i class="fa-solid fa-rotate-left"></i></a>
      </div>
    </form>
  </div>
</div>
      ...
      <div class="col-md-5">
        ...
        <select id="globalSignerSelect" class="form-select border-primary shadow-sm" required>
          ...
          <?php $__currentLoopData = ($signers ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            ?>
            <option value="<?php echo e($s->id); ?>" <?php if($isJarwoko): echo 'selected'; endif; ?>><?php echo e($s->name); ?> (Kode: <?php echo e($s->code); ?>)</option>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </select>
        ...
        <?php $__empty_1 = true; $__currentLoopData = ($certificates ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <tr>
            <td class="text-center py-3">
              <input class="form-check-input rowCheck shadow-sm" type="checkbox" value="<?php echo e($c->id); ?>">
            </td>
            <td class="py-3">
               <div class="fw-semibold text-primary" style="font-size: 0.9em;">
                 <?php echo e($c->certificate_number ?? $c->certificate_no ?? '-'); ?>

               </div>
            </td>
            <td class="py-3">
              <div class="fw-bold text-dark"><?php echo e($c->participant?->name ?? '-'); ?></div>
            </td>
            <td class="py-3">
              <div class="small text-muted"><?php echo e($c->event?->name ?? '-'); ?></div>
            </td>
            <td class="py-3">
              <span class="badge bg-success bg-opacity-10 text-success border border-success-subtle rounded-pill">
                <?php echo e(strtoupper((string) $c->status)); ?>

              </span>
            </td>
            <td class="text-end py-3">
              <div class="d-inline-flex gap-2 justify-content-end">
                <a href="<?php echo e(route('admin.tte.signing.preview', $c->id)); ?>"
                   class="btn btn-light btn-sm rounded-3 border bg-white shadow-sm"
                   target="_blank" title="Cek Dokumen (Preview PDF)">
                   <i class="fa-solid fa-eye text-secondary"></i>
                </a>

                <button type="button" class="btn btn-primary btn-sm rounded-3 shadow-sm btnSingleDispatch fw-semibold" data-id="<?php echo e($c->id); ?>" title="Langsung bubuhkan Signer ke sertifikat ini">
                  Dispatch Sign
                </button>
              </div>
            </td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.btnSingleDispatch').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            dispatchForm.action = '<?php echo e(url('admin/tte/signing')); ?>/' + certId + '/dispatch';
            dispatchForm.submit();
    ...
    if(btnBulk) {
        btnBulk.addEventListener('click', () => {
            ...
            dispatchForm.action = '<?php echo e(route('admin.tte.signing.dispatchBulk')); ?>';
            dispatchForm.submit();
