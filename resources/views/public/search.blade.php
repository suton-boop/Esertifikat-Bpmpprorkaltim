...
@section('title', 'Cari Sertifikat - BPMP Kaltim')
@section('breadcrumb', 'Pencarian Sertifikat')
    ...
    <div class="col-lg-10">
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4 bg-white">
            ...
            <div class="card-body p-4 text-center">
                ...
                <form action="{{ route('public.search.process') }}" method="POST" class="d-flex justify-content-center">
                    @csrf
                    <div class="input-group input-group-lg shadow-sm" style="max-width: 650px;">
                        <input type="text" name="keyword" class="form-control" style="border: 2px solid #e1e8ed;" placeholder="Tulis disini........" required autofocus value="{{ $keyword ?? old('keyword', request('keyword')) }}">
        ...
        @if(isset($results))
            @if(count($results) === 0)
                <div class="alert alert-danger text-center p-4 rounded-3 shadow-sm border-0" style="background-color: #f8dede; color: #a94442;">
                    ...
                    <a href="{{ route('public.search') }}" class="btn btn-warning btn-sm text-white fw-bold px-3 rounded-2 shadow-sm" style="background-color: #f0ad4e; border-color: #eea236;">
                ...
                <div class="card border-0 shadow-sm rounded-3 overflow-hidden mt-4">
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle mb-0 table-bordered border-white">
                            ...
                            <tbody>
                                @foreach($results as $index => $cert)
                                    <tr class="bg-white">
                                        <td class="text-center fw-bold text-dark border-bottom">{{ $index + 1 }}</td>
                                        <td class="fw-semibold text-dark border-bottom">{{ $cert->participant->name }}</td>
                                        <td class="text-dark border-bottom">{{ $cert->participant->institution ?? '-' }}</td>
                                        ...
                                        <td class="text-center border-bottom">
                                            @if($cert->pdf_path || $cert->signed_pdf_path)
                                                <a href="{{ route('public.download', $cert->verify_token) }}" target="_blank" class="btn btn-success btn-sm rounded-1 shadow-sm fw-semibold" style="background-color: #5cb85c; border-color: #4cae4c; font-size: 13px;">
