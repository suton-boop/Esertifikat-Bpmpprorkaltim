            ...
            <div class="card-body p-4 p-md-5 text-center">
                ...
                <form action="{{ route('public.search.process') }}" method="POST" class="d-flex justify-content-center">
                    ...
                    <div class="input-group input-group-lg shadow-sm rounded-3 overflow-hidden" style="max-width: 700px; border: 1px solid #e2e8f0;">
                        ...
                        <input type="text" name="keyword" class="form-control border-0 py-3" placeholder="Contoh: Budi Santoso" required autofocus value="{{ $keyword ?? old('keyword', request('keyword')) }}">
        ...
        @if(isset($results))
            @if(count($results) === 0)
                <div class="alert alert-warning text-center p-5 rounded-4 shadow-sm border-0 bg-white" data-aos="zoom-in">
                    ...
                    <a href="{{ route('public.search') }}" class="btn btn-outline-primary fw-bold px-4 rounded-3">
                            ...
                            <tbody class="bg-white">
                                @foreach($results as $index => $cert)
                                    <tr class="hover-row">
                                        ...
                                        <td>
                                            <div class="fw-bold text-dark">{{ $cert->participant->name }}</div>
                                            <div class="small text-muted">{{ $cert->participant->email ?? '' }}</div>
                                        </td>
                                        <td>
                                            <div class="text-dark">{{ $cert->participant->institution ?? '-' }}</div>
                                        </td>
                                        <td>
                                            <div class="fw-semibold text-dark mb-1">{{ $cert->event->name }}</div>
                                        ...
                                        <td class="text-center">
                                            @if($cert->pdf_path || $cert->signed_pdf_path)
                                                <a href="{{ route('public.download', $cert->verify_token) }}" target="_blank" class="btn btn-sm btn-success rounded-3 px-3 fw-bold d-inline-flex align-items-center gap-1">
